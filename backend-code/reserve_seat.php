...
if((isset($_POST['islogin']))&&(isset($_POST['type']))&&($_POST['islogin']==true)&&($_POST['type']==1)&&(isset($_POST['s_id']))&&(isset($_POST['b_id']))&&(isset($_POST['pickup_stop_id'])))
{
	...
	{
		$response['status']						= 'error';
		$response['message']					= 'You miss it! Bus has gone when ou checked.';
		echo json_encode($response);
	}
	else if($pickup_stop_id==$current_stop)
	{
		$response['status']						= 'success';
		$response['message']					= "Your seat has been reserved.<br> Your bus no '$bus_no' is at your stop i.e. '$bus_stop_name' <br> Get your bus please ";
		echo json_encode($response);
	}
	else
	{
		$multiple_req="";
		$sql_1			="SELECT * FROM reservations where s_id='$s_id'";
		$result1 		= mysqli_query($conn, $sql_1);
		if (mysqli_num_rows($result1) > 0) 
		{
			$response['status']  = 'error';
			$response['message'] = "Your alread send a request for reservation.";
			echo json_encode($response);
		}
		else
		{
			$sql_insert="INSERT INTO reservations(s_id, b_id,pickup_stop) VALUES ('$s_id','$b_id','$pickup_stop_id')";
			...
			$response['status']	 = "success";
			$response['message'] = $multiple_req." Your seat has been reserved.<br> Your bus no $bus_no is at $bus_stop_name <br> Your bus next stop is $next_stop.<br> Your pickup stop is $pickup_stop_name";
			echo json_encode($response);
		}
	}
}
