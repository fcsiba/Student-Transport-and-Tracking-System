...
if ($_SERVER['REQUEST_METHOD'] == 'POST') 
{
	...
	if (isset($_POST['add'])) 
	{
		...
		{
			$sql = "UPDATE `users` SET `name`='$name',`cnic`='$cnic',`password`='base64_encode($password)',`latitude`='$latitude',`longitude`='$longitude',`contact_no`='$contact_no',`address`='$address',`father_name`='$father_name',`father_contact_no`='$father_contact_no',`reg_no`='',`type`='1',`status`='1' WHERE id='$uid'";
		  ...
		  <div class="col-md-12" style="margin-top:2rem;">
            <div class="card card-primary">
			  <div class="card-header" style="background-color: black;">
                <h3 class="card-title"><b> Edit Bus Driver</b></h3>
			  </div>
			  <?php if($check==1){ ?>
              ...
              <form role="form"   action="edit-user-profile.php"  method="POST">
				...
				<div class="card-footer">
                  	<button type="submit" name="add" class="btn btn-primary">Edit Driver Record</button>
                </div>
              </form>
            </div>
		  </div>
